...
class CLoginPlayerServer :
	public CBusiness
{
public:
	CLoginPlayerServer(unsigned count) :CBusiness() {
		m_count = count;
	}
	~CLoginPlayerServer() {
		if (m_db) {
	...
	virtual int BusinessProcess(CProcess* proc) {
		...
		ERR_RETURN(ret, -3);
		ret = setConnectedCallback(&CLoginPlayerServer::Connected, this, _1);
		ERR_RETURN(ret, -4);
		ret = setRecvCallback(&CLoginPlayerServer::Received, this, _1, _2);
		ERR_RETURN(ret, -5);
		...
		for (unsigned i = 0; i < m_count; i++) {
			ret = m_pool.AddTask(&CLoginPlayerServer::ThreadFunc, this);
			ERR_RETURN(ret, -8);
		}
		int sock = 0;
		sockaddr_in addrin;
...
private:
	...
	int HttpParser(const Buffer& data) {
		...
		if (parser.Method() == HTTP_GET) {
			//get 处理
			UrlParser url("https://192.168.43.100" + parser.Url());
			int ret = url.Parser();
			if (ret != 0) {
				TRACEE("ret = %d url[%s]", ret, "https://192.168.43.100" + parser.Url());
				return -2;
			}
			Buffer uri = url.Uri();
	...
	Buffer MakeResponse(int ret) {
		...
		Buffer Server = "Server: Login/1.0\r\nContent-Type: text/html; charset=utf-8\r\nX-Frame-Options: DENY\r\n";
